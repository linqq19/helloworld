...
private:
    ...
    const static int arrayLength = 10;              // 线头绘制范围 - 长度
...
#define OPTIMIZATION 0 // 类型优化，减少switch语句，1开启*9/
    ...
    LINE_HEAD_TYPE endLineHeadType = LINE_HEAD_TYPE::ARROW0;     // 结束点线头方向
...
public:
    Chart_Line(QWidget *parent = nullptr, PAINT_CHART_TYPE type = PAINT_CHART_TYPE::LINE) : Chart_Base(parent,type,true,false,2,0,0)
    {
    ...
    Chart_Line( int x1, int y1, int x2, int y2, QWidget *parent = nullptr,PAINT_CHART_TYPE type = PAINT_CHART_TYPE::LINE) : Chart_Base(x1,y1,x2,y2,parent,type)
    {
        chartText.textType1->setAttribute(Qt::WA_StyledBackground,true);
        chartText.textType1->setAutoFillBackground(true);
        chartText.textType1->setPalette(QPalette(QPalette::Background, QColor(255,255,255,150)));
