...
#include"class_Dialog/dlg_modelinfo.h"
...
private:
    ...
    bool showAll = true;    // 是否显示大小控制点和磁力点
    bool showMag = false;   // 是否显示磁力点
    ...
    PAINT_CHART_TYPE chartType = PAINT_CHART_TYPE::NONE;    // 图形类型
    int ID;         // 图形编号
    qreal portLen = 8;    //端口线的长度 ----------------------------------------------------
    ...
    void paintPorts(QPainter& p);               // 绘制端口------------------------------------------------------------
...
protected:
    class ModelInfo // 模型基本信息 ----------------------------------------------------------------------------------
    {
    public:
        ModelInfo(const QString& name = QString("untitled"), int nIn=1, int nOut=1) :myName(name),numIn(nIn), numOut(nOut) {};
        QString myName;
        int numIn;
        int numOut;
    };
    ...
    ModelInfo model;           // 模型信息---------------------------------------------------------------------------
    dlg_modelinfo* editModel = nullptr;  // 编辑模型信息---------------
...
public:
    ...
    explicit Chart_Base(QWidget *parent = nullptr, PAINT_CHART_TYPE type = PAINT_CHART_TYPE::NONE, bool textType = false, bool mov = true,int spc = 4, int nic = 1, int noc = 1);  // nic输入点个数，输出点个数
    Chart_Base( int x, int y, int w, int h, QWidget *parent = nullptr, PAINT_CHART_TYPE type = PAINT_CHART_TYPE::NONE);
...
public:
    inline qreal getPortLen() { return portLen; }
//    void setGeometryNew(int x,int y,int w, int h);  // 设置位置、大小、更新数据
    ...
    void setMovable(bool f);            // 设置可否移动位置
