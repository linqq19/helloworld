...
Chart_Base::Chart_Base(QWidget *parent, PAINT_CHART_TYPE type, bool textType, bool mov, int spc, int nic, int noc):QWidget(parent),magPoint(nic+noc),sizePoint(spc),movable(mov),chartText(textType)
{
    model.numIn = nic;
    model.numOut = noc;
    chartType = type;
...
Chart_Base::Chart_Base( int x, int y, int w, int h, QWidget *parent, PAINT_CHART_TYPE type):QWidget(parent),magPoint(4),sizePoint(4)
{
    chartType = type;
...
void Chart_Base::pointInit()  // 控制点和磁力点初始化
{
...
{
    if (chartType == PAINT_CHART_TYPE::RECT)   //矩形图形绘制--------------------------------------------------------------------
    {
        ...
        for (int i = 0; i < model.numOut; ++i)
        {
            ...
            magPoint.i_point[index]->setX(paintEnd.rx() + 2 * portLen);
...
void Chart_Base::paintPorts(QPainter& p)
{
    QPen pen(Qt::black);
    pen.setWidth(2);
    p.setPen(pen);
    ...
    //绘制输入端口
    qreal iHeight = height/ (1 + double(model.numIn));  
    for (int i = 1; i <= model.numIn; ++i)
    {
        QPointF end = paintStart + QPointF(0, iHeight * i);
        QPointF top = end + QPointF(-portLen, portLen / 1.73);
        QPointF bottom = end + QPointF(-portLen, -portLen / 1.73);
        p.drawLine(end, top);
        p.drawLine(end, bottom);
    }

    //绘制输出端口
    qreal oHeight = height / (1 + double(model.numOut));
    for (int i = 1; i <= model.numOut; ++i)
    {
        QPointF start(paintEnd.rx(),paintStart.ry()+oHeight*i);
        QPointF end = start + QPointF(2 * portLen, 0);
        QPointF top = end + QPointF(-portLen, portLen / 2);
        QPointF bottom = top + QPointF(0, -portLen);
...
{
    fout.writeRawData(reinterpret_cast<const char*>(&cb.chartType),sizeof(PAINT_CHART_TYPE));
...
void Chart_Base::paintEvent(QPaintEvent *event)
{
    ...
    if (chartType == PAINT_CHART_TYPE::RECT)
    {
        paintPorts(p);
    }
    event->accept();
